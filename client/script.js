...
sayHelloButton.addEventListener("mouseout",resetbtncolor)
...
const repeatMyParam = () => {
    axios.get('http://localhost:3000/repeat/melissa').then((res) => {
...
const createFood = (event) => {
    ...
    axios.post('http://localhost:3000/food',body).then((res) => {
        console.log(res.data)
        //so it prints without deleting the whole page
        const paragraph = document.createElement("p")
